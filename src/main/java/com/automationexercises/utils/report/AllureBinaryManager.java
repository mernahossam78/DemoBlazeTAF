...
//that code's job is to download the allure from its repo. and add it in my path
public class AllureBinaryManager {
    public static void downloadAndExtract() {
        try {
            ...
            Files.deleteIfExists(Files.list(AllureConstants.EXTRACTION_DIR).filter(p -> p.toString().endsWith(".zip")).findFirst().orElse(null));
